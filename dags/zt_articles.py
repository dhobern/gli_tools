            ...
            with open(file_new, newline='', encoding='utf8') as f:
                ...
                for issue in reader:
                    ...
                    for article in articles:
                        title = article.xpath(".//h2[@class='title']//a")[0].text.strip().replace("!@", "<").replace("@!", ">").replace("  ", " ")
                        if "lepidoptera" in title.lower():
                            url = article.xpath(".//h2[@class='title']//a")[0].attrib["href"].strip()
                            authors = article.xpath(".//div[@class='authors']")[0].text.strip()
                            pages = article.xpath(".//div[@class='pages']")[2].text.strip()     
                            images = article.xpath(".//img")
                            if images is not None and len(images) > 0: 
                                image = images[0].attrib["src"].strip()
                            else:
                                image = ""
                            writer.writerow(["Zootaxa", url, title, authors, issue[1], pages, image])