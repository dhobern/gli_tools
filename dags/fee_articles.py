...
def get_fee_articles():
    ...
    with open(file_latest, "w", newline='', encoding='utf8') as f:
        ...
        html = page.read().decode("utf-8").replace("<strong>", "").replace("</strong>", "||").replace("<em>", "!@em@!").replace("</em>", "!@/em@!")
        ...
        for node in container.iter():
            if node.tag == 'h2':
                ...
                title = parts[1].strip().replace("!@", "<").replace("@!", ">").replace("\n", " ").replace("  ", " ")
                if "lepidoptera" in title.lower():
                    authors = parts[0].strip()
                    url = "https://www.biosoil.ru" + node.xpath(".//a")[0].attrib["href"].strip()
                    pages = ""
                    image = ""
                    record = ["Far Eastern Entomologist", url, title, authors, issue, pages, image]
